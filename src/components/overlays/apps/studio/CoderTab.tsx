...
const updateFileContent = (path: string, newContent: string, fs: any): any => {
    const newFs = JSON.parse(JSON.stringify(fs)); // Deep clone
    const parts = path.split('/').filter(p => p);
    let current = newFs;
...
const CoderTab: React.FC = () => {
    ...
    return (
        <div className="w-full h-full flex font-mono bg-[#2d2d2d]">
            <FileExplorer fileSystem={fileSystem} onSelectFile={handleSelectFile} activeFile={activeFile} />
            <div className="flex-1 flex flex-col">
                 <div className="p-2 bg-brand-surface border-b border-brand-border flex items-center justify-between">
                     <div role="tablist" aria-label="Open files" className="flex items-center gap-1">
                ...
                <div
                    ...
                    role="tabpanel"
                    hidden={!activeFile}
                    aria-labelledby={openFiles.length > 0 && activeFile ? `file-tab-${openFiles.indexOf(activeFile)}` : undefined}
                ...
                >
                    {activeFile ? (
                        <Editor
                            ...
                            highlight={c => highlight(c, getPrismLanguage(activeFileExtension), activeFileExtension)}
                            padding={10}
                            className="text-sm"
                            style={{
                        ...
                        <div className="p-4 text-center text-gray-400">Select a file to begin editing.</div>
                    )}
                </div>
            </div>
        </div>
