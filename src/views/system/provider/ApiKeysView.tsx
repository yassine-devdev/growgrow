...
const KeyManagementView: React.FC = () => {
    ...
    const { data, isLoading } = useQuery<PaginatedResponse<ApiKey>>({ 
        ...
        queryFn: () => getApiKeys({ pageIndex: pagination.pageIndex, pageSize: pagination.pageSize, sorting, globalFilter }),
        placeholderData: (prev) => prev
    });
    
    const columns: { accessorKey: Extract<keyof ApiKey, string>; header: string; cell?: (value: any, item: ApiKey) => ReactNode; }[] = useMemo(() => [
    ...
    return (
        <Suspense fallback={<LoadingSpinner />}>
            <Routes>
                ...
                <Route path="key-permissions" element={<EmptyState message="Granular API key permission management is coming soon." />} />
                <Route path="key-rotation" element={<EmptyState message="Automated API key rotation policies will be available here." />} />
            </Routes>
        </Suspense>
    );
};
