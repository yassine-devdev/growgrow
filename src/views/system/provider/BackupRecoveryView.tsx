...
const BackupHistoryView: React.FC = () => {
    ...
    const { data, isLoading } = useQuery<PaginatedResponse<Backup>>({ 
        ...
        queryFn: () => getBackups({ pageIndex: pagination.pageIndex, pageSize: pagination.pageSize, sorting, globalFilter }),
        placeholderData: (prev) => prev
    });
    
    const columns: { accessorKey: Extract<keyof Backup, string>; header: string; cell?: (value: any, item: Backup) => ReactNode }[] = useMemo(() => [
        { accessorKey: 'date', header: 'Date', cell: (date: string) => new Date(date).toLocaleString() },
    ...
    return (
        <Suspense fallback={<LoadingSpinner />}>
            <Routes>
                ...
                <Route path="recovery-plans" element={<EmptyState message="Disaster recovery plan management is coming soon." />} />
                <Route path="restore-tools" element={<EmptyState message="Point-in-time restore tools will be available here." />} />
            </Routes>
        </Suspense>
    );
};
