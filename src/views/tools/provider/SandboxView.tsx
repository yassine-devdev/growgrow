...
const BetaFeatures = lazy(() => import('./sandbox/BetaFeatures'));
const ApiTesting = lazy(() => import('./sandbox/ApiTesting'));
const VectorbaseCreator = lazy(() => import('./sandbox/VectorbaseCreator'));
...
const SandboxView: React.FC = () => {
    ...
    const sandboxNav = NAVIGATION_CONFIG[user!.role]
        .find(m => m.id === 'tools')
        ?.headerNav.find(h => h.id === 'sandbox');
        
    return (
        <div className="h-full flex flex-col gap-4">
            <nav className="flex items-center gap-1 bg-black/5 dark:bg-white/5 p-1 rounded-xl">
                 {sandboxNav?.children?.map(item => (
                    <NavLink
                        key={item.id}
                        to={`/tools/sandbox/${item.id}`}
            ...
            <div className="flex-1 overflow-y-auto pr-2">
                <Suspense fallback={<LoadingSpinner />}>
                    <Routes>
                        <Route index element={<Navigate to="beta-features" replace />} />
                        <Route path="beta-features" element={<BetaFeatures />} />
                        <Route path="api-testing" element={<ApiTesting />} />
                        <Route path="vector-database" element={<VectorbaseCreator />} />
                    </Routes>
                </Suspense>
            </div>
        </div>
