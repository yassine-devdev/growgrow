...
import InvoiceFormModal from '@/views/tools/provider/finance/InvoiceFormModal';
...
const ReportsView: React.FC = () => {
    ...
    const { data, isLoading } = useQuery<PaginatedResponse<Invoice>>({
        queryKey: ['invoices_reports', pagination, sorting, globalFilter],
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['invoices_reports'] });
        },
        onError: (error: Error) => {
    ...
    return (
        <div className="h-full flex flex-col gap-4">
            <h1 className="text-xl font-bold text-brand-text">Financial Reports: Invoices</h1>
            <DataTable 
                data={data?.rows ?? []} 
                columns={columns} 
