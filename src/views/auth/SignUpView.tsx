...
const SignUpView: React.FC<SignUpViewProps> = ({ onSwitchToSignIn }) => {
  ...
  const signUpMutation = useMutation({
    ...
    onError: (error: any) =>
      addToast({ message: error.message || "Sign up failed", type: "error" }),
  });
  ...
  const handleBiometricRegistration = async () => {
    try {
      ...
      console.error(error);
      addToast({
        message: error.message || "Biometric registration failed.",
        type: "error",
      });
        ...
        return (
          ...
          >
            ...
            <p className="text-sm text-brand-text-alt">
              Enter the 6-digit code we sent to your phone number to complete
              verification.
            </p>
            <ControlledInput
              control={otpControl}
      ...
      case "biometric":
        return (
          <div className="text-center space-y-6">
            ...
            <p className="text-sm text-brand-text-alt">
              For faster, more secure access, you can set up Face or Fingerprint
              ID.
            </p>
            <button
            ...
            <button
              ...
              className="w-full text-sm font-semibold text-brand-text-alt hover:underline"
            >
              Skip for now
            </button>
          </div>
      ...
      case "complete":
        return (
          <div className="text-center space-y-6">
            ...
            <button onClick={onSwitchToSignIn} className="w-full btn-primary">
              Go to Sign In
            </button>
          </div>
        );
  ...
  return (
    <AuthCard>
      ...
      <p className="text-center text-brand-text-alt mb-6">
        Join the GROW YouR NEED universe.
      </p>
