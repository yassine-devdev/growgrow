...
const SignInView: React.FC<SignInViewProps> = ({ onSwitchToSignUp }) => {
  ...
  const loginMutation = useMutation({
    mutationFn: (data: SignInFormData) => loginUser(data),
  ...
  const otpVerifyMutation = useMutation({
    mutationFn: (data: { phone: string; otp: string }) =>
      verifyPhoneOtp(data.phone, data.otp),
    onSuccess: (data) => login(data.user),
    onError: (error: any) =>
      addToast({
      ...
      return (
        <AnimatePresence mode="wait">
          {otpStep === "phone" ? (
            ...
            >
              <form
                onSubmit={handlePhoneSubmit(onRequestOtp)}
                className="space-y-6"
              >
            ...
            >
              <button
                ...
                className="flex items-center gap-2 text-sm text-brand-text-alt mb-4"
              >
                <ArrowLeft size={16} /> Back
              </button>
              <form
                onSubmit={handleOtpVerifySubmit(onVerifyOtp)}
                className="space-y-6"
              >
      ...
      return (
        <div className="text-center space-y-6">
          ...
          <p className="text-sm text-brand-text-alt">
            Use your registered fingerprint or face to sign in instantly.
          </p>
          <button
    ...
    return (
      ...
      >
        <ControlledInput
          ...
          error={passwordErrors.identifier}
        />
        <ControlledInput
          control={passwordControl}
          name="password"
  ...
  return (
    <AuthCard>
      ...
      <p className="text-center text-brand-text-alt mb-6">
        Sign in to continue to your universe.
      </p>
