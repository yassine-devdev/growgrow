...
import { getAssignmentDetails, submitAssignment } from '../../../api/studentApi';
import { QUERY_KEYS } from '../../../constants/queries';
import { Loader2, Info, CheckCircle, UploadCloud, FileText, Trash2 } from 'lucide-react';
...
const AssignmentView: React.FC = () => {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (newSubmission) => {
            queryClient.setQueryData(
                ...
                (oldData: AssignmentDetails | undefined) => oldData ? { ...oldData, submission: newSubmission } : oldData
            );
            addToast({ message: 'Assignment submitted successfully!', type: 'success' });
        },
        onError: (error: Error) => {
            addToast({ message: error.message || 'Submission failed.', type: 'error' });
        }
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!assignmentId || (!text && !file)) {
            addToast({ message: 'Please provide a text response or a file.', type: 'error' });
            return;
        }
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold text-brand-text mb-1">{assignment.title}</h1>
            <p className="text-sm text-brand-text-alt mb-6">{t('views.assignmentView.course')}: {assignment.course}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="p-6 bg-brand-surface-alt/50 border border-brand-border rounded-lg">
                        ...
                        <p className="text-brand-text-alt whitespace-pre-wrap">{assignment.instructions}</p>
                    </div>
                    ...
                    {isSubmitted ? (
                        <div className="p-6 bg-green-500/10 border border-green-500/20 rounded-lg">
                            ...
                            <p className="font-semibold">{t('views.assignmentView.status')}: <span className="font-normal">{t('views.assignmentView.statusSubmittedOn', { date: new Date(assignment.submission.submittedAt).toLocaleString() })}</span></p>
                            {assignment.submission.submittedText && (
                                <div className="mt-2">
                                    <h3 className="font-semibold text-sm">{t('views.assignmentView.submittedText')}:</h3>
                                    <p className="text-sm p-2 bg-brand-surface rounded-md mt-1">{assignment.submission.submittedText}</p>
                                </div>
                            )}
                            {assignment.submission.submittedFile && (
                                <div className="mt-2">
                                    <h3 className="font-semibold text-sm">{t('views.assignmentView.submittedFile')}:</h3>
                                    <div className="flex items-center gap-2 text-sm p-2 bg-brand-surface rounded-md mt-1">
                                        <FileText className="w-4 h-4 text-brand-primary" />
                                        <span>{assignment.submission.submittedFile.name} ({(assignment.submission.submittedFile.size / 1024).toFixed(2)} KB)</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-6 bg-brand-surface-alt/50 border border-brand-border rounded-lg">
                        <h2 className="text-lg font-semibold text-brand-text mb-4">{isSubmitted ? t('views.assignmentView.updateButton') : t('views.assignmentView.submitWork')}</h2>
                        <div className="space-y-4">
                            ...
                            />
                             <div>
                                <label className="block text-sm font-medium text-brand-text-alt">{t('views.assignmentView.fileUploadLabel')}</label>
                                {file ? (
                                    <div className="mt-2 flex items-center justify-between p-3 bg-brand-surface rounded-lg border border-brand-primary">
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            ...
                                            <div className="overflow-hidden">
                                                ...
                                                <p className="text-xs text-brand-text-alt">{(file.size / 1024).toFixed(2)} KB</p>
                                            </div>
                                        </div>
                                        <button
                                            type="button"
                                        ...
                                        <div className="text-center">
                                            ...
                                            <div className="mt-4 flex text-sm leading-6 text-brand-text-alt dark:text-dark-text-alt">
                                                ...
                                                >
                                                    ...
                                                    <input id="file-upload" name="file-upload" type="file" className="sr-only" onChange={handleFileChange} disabled={mutation.isPending} />
                                                </label>
                                                <p className="pl-1">or drag and drop</p>
                                            </div>
                                            <p className="text-xs leading-5 text-brand-text-alt/80 dark:text-dark-text-alt/80">PNG, JPG, PDF up to 10MB</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                         ...
                         <div className="mt-6 flex justify-end">
                            ...
                            >
                                ...
                                {mutation.isPending ? t('views.assignmentView.submittingButton') : (isSubmitted ? t('views.assignmentView.updateButton') : t('views.assignmentView.submitButton'))}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <aside className="p-4 bg-brand-surface-alt/50 border border-brand-border rounded-lg self-start">
                    <p><strong>{t('views.assignmentView.dueDate')}:</strong></p>
