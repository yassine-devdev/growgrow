...
// FIX: Correct the import path to point to the correct database service inside the src directory.
import * as db from '../src/db/database.service';
import { GoogleGenAI } from '@google/genai';
import config from '../../config';

// A generic controller to handle all data requests
const handleRequest = (handler: Function): RequestHandler => async (req, res, next) => {
    try {
...
export const getProviderSchoolDetails: RequestHandler = async (req, res, next) => {
    try {
...
export const getVersionControlCommits = handleRequest(db.getVersionControlCommits);
export const getAdminDashboardData = handleRequest(db.getAdminDashboardData);
export const getTeacherDashboardData = handleRequest(db.getTeacherDashboardData);
export const getStudentDashboardData = handleRequest(db.getStudentDashboardData);
export const getParentDashboardData = handleRequest(db.getParentDashboardData);
export const getAdmissionsDashboardData = handleRequest(db.getAdmissionsDashboardData);
export const getIndividualDashboardData = handleRequest(db.getIndividualDashboardData);
...
export const getMultiTenancySettings = handleRequest(db.getMultiTenancySettings);
...
export const getPredictiveAnalyticsData = handleRequest(db.getPredictiveAnalyticsData);
export const getCohortAnalysisData = handleRequest(db.getCohortAnalysisData);
export const getProviderFinanceDashboardData = handleRequest(db.getProviderFinanceDashboardData);


export const saveMultiTenancySettings: RequestHandler = async (req, res, next) => {
    try {
...
export const deleteMarketingCampaign: RequestHandler = async (req, res, next) => {
    try {
...
export const getAiLessonPlan: RequestHandler = async (req, res, next) => {
    try {
        ...
        const result = await ai.models.generateContent({ model: 'gemini-2.5-flash', contents: prompt });
        const resultText = result.text.replace(/```json|```/g, '').trim();
...
export const getStudioDesignTemplates = handleRequest(db.getStudioDesignTemplates);
...
export const getStudioMarketplaceAssets = handleRequest(db.getStudioMarketplaceAssets);
...
// HRM Controllers
export const getTeamMembers = handleRequest(db.getTeamMembers);
export const getRolesAndPermissions = handleRequest(db.getRolesAndPermissions);
export const getAllPermissions = handleRequest(db.getAllPermissions);
