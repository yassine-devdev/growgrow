...
class AIGatewayService {
  public async generateResponseStream(
    ...
    const promptEmbedding =
      ...
      Array.isArray(embeddingResponse.embeddings[0].values)
        ? embeddingResponse.embeddings[0].values
        : undefined;

    const contextChunks = promptEmbedding
      ? await vectorService.similaritySearch(promptEmbedding, 3)
      : [];
...
`;
    ...
    const provider = modelRouter.selectModel({
      taskType: "chat",
      intent: "balanced",
    });
