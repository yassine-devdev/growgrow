...
interface RoutingCriteria {
    taskType: 'chat' | 'summarization' | 'data_analysis' | 'image_generation' | 'code_generation';
...
class ModelRouter {
    ...
    private initializeHealthChecks() {
        ...
        this.providerHealth.set(this.geminiProvider, { isHealthy: true, latency: 1500 });
        this.providerHealth.set(this.openAIProvider, { isHealthy: true, latency: 2000 });
        this.providerHealth.set(this.anthropicProvider, { isHealthy: true, latency: 2200 });
        this.providerHealth.set(this.groqProvider, { isHealthy: true, latency: 300 });
        this.providerHealth.set(this.openRouterProvider, { isHealthy: true, latency: 2500 });
        // Simulate an outage for a provider to test resilience
        this.providerHealth.set(this.ollamaProvider, { isHealthy: false, latency: 99999 });
    }
    ...
    selectModel(criteria: RoutingCriteria): IAIProvider {
        ...
        if (eligibleProviders.length > 0) {
            ...
            logger.info(`Routing decision: Matched rule. Selected provider by performance.`, { 
        ...
        if (process.env.USE_OLLAMA === 'true' && this.providerHealth.get(this.ollamaProvider)?.isHealthy) {
            logger.info('Routing decision (Fallback): Using Ollama for local development.');
            return this.ollamaProvider;
        }

        logger.info(`Routing decision (Fallback): No specific rule matched or provider unhealthy. Using default provider: GeminiProvider`);
