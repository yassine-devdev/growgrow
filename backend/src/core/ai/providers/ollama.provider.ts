...
export class OllamaProvider implements IAIProvider {
    ...
    async generateStream(prompt: string): Promise<ReadableStream<string>> {
        logger.info(`[OllamaProvider] Generating stream with model: ${DEFAULT_MODEL}`);
        ...
        const transformStream = new TransformStream<Uint8Array, string>({
            transform(chunk, controller) {
                ...
                const jsonStrs = jsonObjectsStr.split('\n').filter(s => s.trim() !== '');
                ...
                for (const jsonStr of jsonStrs) {
                    try {
                        ...
                        logger.error(`[OllamaProvider] Error parsing stream chunk: ${jsonStr}`, error as Error);
                    }
                }
            }
        });
        ...
        (async () => {
            try {
                ...
                logger.error('[OllamaProvider] Fetch or pipe error:', error as Error);
