...
export const getDataConnectors = async () =>
  Promise.resolve(mockDataConnectors);
...
const mockSchemas = {
  PostgreSQL: {
    columns: ["user_id", "created_at", "plan_type", "monthly_spend"],
  },
  "Google Analytics": {
    columns: [
      "ga:date",
  ...
  Stripe: {
    columns: ["id", "amount", "created", "currency", "customer", "description"],
  },
  "CSV Upload": { columns: [] },
  "SQL Database": { columns: [] },
};

export const getConnectorSchema = async (type: string) =>
  Promise.resolve(
    mockSchemas[type as keyof typeof mockSchemas] || { columns: [] }
  );
