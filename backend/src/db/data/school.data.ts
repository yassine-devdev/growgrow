...
import { LessonPlan, LessonPlanFormData } from '../../../../../api/schemas/schoolHubSchemas';
import { AdminDashboardData, TeacherDashboardData, StudentDashboardData, ParentDashboardData, AdmissionsDashboardData } from '../../../../../api/schemas/dashboardSchemas';
import { AcademicHealthData } from '../../../../../api/schemas/adminSchemas';
import { v4 as uuidv4 } from 'uuid';

// --- In-memory store for Lesson Plans ---
let lessonPlansStore: LessonPlan[] = [
    { id: 'lp-1', title: 'Introduction to Algebra', course: 'Algebra II', date: '2024-09-05', objectives: 'Understand variables and expressions.', materials: 'Whiteboard, markers, textbook', activities: '- Lecture on variables\n- Group work on practice problems\n- Exit ticket quiz' },
    { id: 'lp-2', title: 'The Roman Empire', course: 'World History', date: '2024-09-06', objectives: 'Identify key figures of the Roman Empire.', materials: 'Slideshow, map, handouts', activities: '- Presentation on Roman history\n- Map labeling activity\n- Class discussion' },
];
...
export const saveLessonPlan = async (planData: LessonPlanFormData & { id?: string }): Promise<LessonPlan> => {
    if (planData.id) { // Update
...
export const deleteLessonPlan = async (id: string): Promise<{ success: boolean }> => {
...
export const getAcademicHealthData = async (): Promise<AcademicHealthData> => ({
    ...
    enrollmentTrend: [
        { month: 'Jan', enrollment: 1390 }, { month: 'Feb', enrollment: 1400 },
        { month: 'Mar', enrollment: 1410 }, { month: 'Apr', enrollment: 1405 },
...
export const getAdminDashboardData = async (): Promise<AdminDashboardData> => ({
    stats: [
        { label: 'Total Students', value: '1,428', change: '+2.1%', icon: 'Users' as const },
        { label: 'Teaching Staff', value: '86', icon: 'GraduationCap' as const },
        { label: 'Avg. Attendance', value: '94.7%', change: '-0.5%', icon: 'UserCheck' as const },
        { label: 'Pending Applications', value: '12', icon: 'UserPlus' as const },
    ],
    enrollmentTrend: [ { name: 'Jan', students: 1390 }, { name: 'Feb', students: 1400 }, { name: 'May', students: 1428 }, ],
...
export const getParentDashboardData = async (): Promise<ParentDashboardData> => ({
    children: [
        { name: 'Alex Doe', stats: [{ label: 'GPA', value: '3.8', icon: 'GraduationCap' as const }] }
    ],
...
export const getAdmissionsDashboardData = async (): Promise<AdmissionsDashboardData> => ({
    stats: [
        { label: 'New Applications', value: '58', change: '+10', icon: 'FilePlus' as const },
        { label: 'Pending Review', value: '12', icon: 'Eye' as const },
        { label: 'Offers Accepted', value: '25', change: '+5%', icon: 'UserCheck' as const },
        { label: 'Enrollment Rate', value: '65%', icon: 'TrendingUp' as const },
    ],
    applicationTrend: [
        { name: 'Jan', applications: 20 }, { name: 'Feb', applications: 35 },
        { name: 'Mar', applications: 40 }, { name: 'Apr', applications: 50 },
    ],
    needsReview: [ { id: 'app-1', name: 'Charlie Brown', submitted: 'Today' } ],
    applicantFunnel: [
        { stage: 'Applied', count: 150 }, { stage: 'Reviewing', count: 58 },
    ],
    demographics: [ { name: 'Domestic', value: 120 }, { name: 'International', value: 30 } ]
});