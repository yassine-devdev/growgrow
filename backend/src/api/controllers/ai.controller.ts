...
export const conciergeChatController: RequestHandler = async (req, res, next) => {
    try {
        ...
        const stream = await aiGatewayService.generateResponseStream(prompt, userRole);
...
export const categorizeExpenseController: RequestHandler = async (req, res, next) => {
    try {
        ...
        if (categories.includes(category)) {
            ...
            res.status(500).json({ message: 'AI failed to return a valid category.' });
        }
    } catch (error) {
        next(error);
    }
